...
function convert() {
	try {
		...
		alert("Invalid FTB code! Please check the help menu for more info.","","")
	}
}
...
function step(){
	if (isLoaded){
		...
		document.getElementById("help").style.left = 850+tPP
		document.getElementById("convertText").style.left = 800+tPP
		document.getElementById("ftbCode").style.left = 800+tPP
		document.getElementById("convert").style.left = 825+tPP
		document.getElementById("toggleHelp").style.left = 800+tPP
		...
		if (mouseX > (window.innerWidth-150) && mouseY > (window.innerHeight-150)) {
			redditPos = (((redditPos-5)/2)+5)
		} else {
			redditPos = (((redditPos+110)/2)-110)
		}
		document.getElementById("Reddit").style.bottom = redditPos
