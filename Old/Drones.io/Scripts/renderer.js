...
function renderObjs() {
	for(var i = 0;i < objects.length; i++){
		...
		ctx.translate(centerOffsetX/azoom+objects[i].x-camX,centerOffsetY/azoom-objects[i].y+camY);
		if (objects[i].type == "AC") {
			//Draw Barrel
			ctx.translate(-32,-64);
			ctx.translate(32,64);
			ctx.rotate(rad(180-pointTo(objects[i].x,objects[i].y,objects[i].ax,objects[i].ay)));
			ctx.drawImage(document.getElementById("AC"),64,0,64,64,-64,-32,64,64);
			
			//Draw Body
			ctx.setTransform(azoom, 0, 0, azoom, 0, 0);
			ctx.translate(centerOffsetX/azoom+objects[i].x-camX,centerOffsetY/azoom-objects[i].y+camY);
			ctx.translate(-32,-64);
			ctx.drawImage(document.getElementById("AC"),0,0,64,64,0,0,64,64);
		};
		ctx.setTransform(1, 0, 0, 1, 0, 0);
		ctx.globalAlpha = 1;
	};
...
function renderProj(){
	for(var i = 0;i < projectiles.length; i++){
		...
		ctx.stroke();
			...
			ctx.setTransform(1, 0, 0, 1, 0, 0);
		} else if (projectiles[i].type=="AC") {
			ctx.translate(tmpX,tmpY);
			ctx.translate(-32,-32);
			ctx.rotate(rad(180+projectiles[i].dir));
			ctx.drawImage(document.getElementById("AC"),0,64,64,64,-64,-32,64,64);
			ctx.setTransform(1, 0, 0, 1, 0, 0);
		};
	};
};
